...
Aptly uses a **two-path authentication model** to solve the bootstrap problem: how do you create API keys without already having an API key?

## Overview

<CardGroup cols={2}>
  <Card title="Admin Secret" icon="user-shield" color="#ea5a0c">
    Server-side secret for **creating customers**
  </Card>
  <Card title="API Keys" icon="key" color="#0285c7">
    Customer keys for **all operations**
  </Card>
</CardGroup>
...
The Admin Secret is a server-side environment variable (`APTLY_ADMIN_SECRET`) used **only** for:

- Creating new customers (`POST /v1/admin/customers`)
- Listing customers (`GET /v1/admin/customers`)
- Viewing customer details (`GET /v1/admin/customers/{id}`)
- Creating API keys for customers (`POST /v1/admin/customers/{id}/api-keys`)

<Warning>
  The Admin Secret should **never** be shared with customers or exposed in client-side code. It's a server-side secret only.
</Warning>

#### Using the Admin Secret

Pass it via the `X-Admin-Secret` header:

```bash
curl -X POST https://api.aptly.dev/v1/admin/customers \
  -H "X-Admin-Secret: your-admin-secret" \
  -H "Content-Type: application/json" \
  -d '{"email": "user@company.com", "company_name": "Company Inc"}'
```

<Tip>
  Generate a secure admin secret with: `openssl rand -base64 32`
</Tip>

### Path 2: Customer API Keys

Once a customer is created, they receive an API key in the format `apt_live_*` or `apt_test_*`. This key is used for **all customer-facing operations**:
...
<AccordionGroup>
  ...
  <Accordion title="Revocation">
    ...
    ```bash
    curl -X DELETE https://api.aptly.dev/v1/api-keys/{key_id} \
...
```bash
curl -X DELETE https://api.aptly.dev/v1/api-keys/key_abc456 \
  -H "Authorization: Bearer apt_live_..."
```
